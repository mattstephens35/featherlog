...
variables → Actions → "New repository secret") containing a Claude Code
OAuth token generated by running `claude setup-token` (Claude Pro/Max
subscription), passed to `claude-code-base-action` via
`claude_code_oauth_token`. Until that secret is added, this workflow will
fail at the "Run Playwright MCP agent tests" step.
